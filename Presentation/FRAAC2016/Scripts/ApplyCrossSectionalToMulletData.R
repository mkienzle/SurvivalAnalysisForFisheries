...
new.data <- data.frame(Year = results$Year, Sector = "Estuarine+Ocean", "Fishing.mortality" = results$Z.est - 0.33)

p2 <- ggplot(data = new.data, aes(x = Year, y = Fishing.mortality, color = Sector))
p2 <- p2 + geom_abline(intercept = 0.33, slope = 0, size = 1.2)
p2 <- p2 + scale_x_continuous(name = "") + scale_y_continuous(name = "Mortality rates (1/year)", limits = c(0, 1)) 
p2 <- p2 + ggtitle("cross sectional analysis")
p2 <- p2 + geom_line(size = 1.5)
p2 <- p2 + theme(legend.position = "bottom", axis.text=element_text(size=12), axis.title=element_text(size=18,face="bold"), legend.text=element_text(size=12), legend.title = element_text(size=14), plot.title = element_text(size=20, face = "bold"))

print(p2)
postscript(file = "../Results/Graphics/CrossSectional-EstimatedTrendsInMortality.ps")
print(p2)
dev.off()
