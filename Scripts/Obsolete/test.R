...
llfunc = function(F){ # a function of only 1 parameter (F)
  ...
  # Finally the likelihood supposing M is given
  P = prob2-prob1
  -sum( catch  * log( P  * M/(M+F) / sum(P * M / (M+F)) ))
...
print(paste("Estimated F is", round(result$par,3), " +- ", round(sqrt(solve(result$hessian)),2)))
