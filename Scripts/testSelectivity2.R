...
catch <- 1/1e4 * c(0,0,308450, 357110, 240461,64508,28809,16295,14423)
#catch <-1/1e4 *  c(0, 0, 140136, 491726, 178049, 94187, 36491)
...
llfunc = function(par){ # a function of only 1 parameter (F)
  ...
  # And total mortality
  #Z = M + F * alpha * age
    selectivity <- c(0,0, alpha, rep(1, length(age[4:8])))
  Z = M + F * selectivity
...
result <- optim(par = c(0.5,0.05), fn = llfunc, method = c("L-BFGS-B"),
      lower = c(1e-2,1e-2), upper = c(1,1), hessian = TRUE)
